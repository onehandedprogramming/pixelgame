use crate::util::{swap_buf::SwapBuffer, vector::Vec2};

pub struct LiquidGrid {
    pub size: Vec2<usize>,
    pub pos: SwapBuffer<Vec2<f32>>,
    pub vel: SwapBuffer<Vec2<f32>>,
    pub dens: SwapBuffer<f32>,
    pub barrier: Vec<bool>,
}

impl LiquidGrid {
    pub fn new(size: Vec2<usize>) -> Self {
        let len = size.area();
        let mut barrier = vec![false; len];
        for x in 0..2 {
            let x = x * (size.x - 1);
            for y in 0..size.y {
                barrier[x + y * size.x] = true;
            }
        }
        for y in 0..2 {
            let y = y * (size.y - 1);
            for x in 0..size.x {
                barrier[x + y * size.x] = true;
            }
        }
        Self {
            size,
            pos: SwapBuffer::new(len),
            vel: SwapBuffer::new(len),
            dens: SwapBuffer::new(len),
            barrier,
        }
    }

    pub fn update(&mut self, dt: f32) {
        let (dr, dw, dm) = self.dens.rwm();
        let (vr, vw, vm) = self.vel.rwm();
        let (pr, pw, pm) = self.pos.rwm();
        for y in 1..self.size.y - 1 {
            let y_add = y * self.size.x;
            for x in 1..self.size.x - 1 {
                let i = x + y_add;
                let mut den = dr[i];
                // if den < 0.001 {
                //     den = 0.0;
                // }
                if den == 0.0 {
                    pw[i] = Vec2::zero();
                    dw[i] = den;
                    vw[i] = Vec2::zero();
                    continue;
                }
                let mut pressure = (den - 0.9).max(0.0);
                let mut vel = vr[i];
                let mut pos = pr[i];
                pos.x = pos.x.clamp(-0.5, 0.5);
                pos.y = pos.y.clamp(-0.5, 0.5);

                let mut new_pos = pos + vel * dt;
                new_pos.x = new_pos.x.clamp(-1.49, 1.49);
                new_pos.y = new_pos.y.clamp(-1.49, 1.49);
                let mut change = new_pos - pos;

                // account for barriers

                // TODO i32 to isize
                let dir: Vec2<i32> = (pos + change + 0.5).floor().into();
                if dir.x != 0 || dir.y != 0 {
                    let ix = (i as i32 + dir.x) as usize;
                    let iy = (i as i32 + dir.y * self.size.x as i32) as usize;
                    let i2 = (iy as i32 + dir.x) as usize;
                    let b = self.barrier[i2] || dr[i2] > den * 1.1;
                    let bx = self.barrier[ix] || dr[ix] > den * 1.1;
                    let by = self.barrier[iy] || dr[iy] > den * 1.1;
                    if b || bx || by {
                        let mut mult = Vec2 {
                            x: (!self.barrier[ix]) as i32 as f32,
                            y: (!self.barrier[iy]) as i32 as f32,
                        };
                        if self.barrier[i2] && mult == (Vec2 { x: 1.0, y: 1.0 }) {
                            mult = Vec2::zero();
                        }
                        pressure += (vel * (Vec2 { x: 1.0, y: 1.0 } - mult)).mag() * (den - 0.1).max(0.0);
                        change = change * mult;
                        vel = vel * mult;
                    }
                }

                // expand

                if pressure > 0.0 {
                    let mut moved = 0.0;
                    for (dirs, mult) in [
                        (Vec2::<i32>::CARDINAL_DIRECTIONS, 1.0 / 9.0),
                        (Vec2::<i32>::CORNERS, 1.0 / 36.0),
                    ] {
                        for dir in dirs {
                            let i2 = (i as i32 + dir.x + dir.y * self.size.x as i32) as usize;
                            if dir.x.abs() == 1 && dir.y.abs() == 1 {
                                let ix = (i as i32 + dir.x) as usize;
                                let iy = (i as i32 + dir.y * self.size.x as i32) as usize;
                                if self.barrier[ix] && self.barrier[iy] {
                                    continue;
                                }
                            }
                            if !self.barrier[i2] && dr[i2] < den {
                                let change = (mult * (den - dr[i2]) * dt * 100.0).min(den * mult);
                                if change > 0.0 {
                                    dm[i2] += change;

                                    let fdir: Vec2<f32> = dir.into();
                                    let fdir = fdir * 2.0;
                                    let sum = change + dr[i2];
                                    vm[i2] += (fdir * change + vr[i2] * dr[i2]) / sum - vr[i2];

                                    moved += change;
                                }
                            }
                        }
                    }
                    den -= moved;
                    pressure -= moved;
                }

                // move

                pos += change;
                let dir: Vec2<i32> = (pos + 0.5).floor().into();
                if dir.x != 0 || dir.y != 0 {
                    let i2 = (i as i32 + dir.x + dir.y * self.size.x as i32) as usize;
                    let move_den = (den * den + vel.mag() * 0.1 - dr[i2] * dr[i2]).max(0.0).min(den);
                    if move_den > 0.0 {
                        let sum = move_den + dr[i2];
                        let mov: Vec2<f32> = dir.into();
                        let new_pos = pos - mov;
                        pm[i2] += (new_pos * move_den + pr[i2] * dr[i2]) / sum - pr[i2];
                        vm[i2] += (vel * move_den + vr[i2] * dr[i2]) / sum - vr[i2];
                        dm[i2] += move_den;

                        den -= move_den;
                    }
                    let dirs = if dir.x == 0 {
                        [dir + Vec2 { x: 1, y: 0 }, dir + Vec2 { x: -1, y: 0 }]
                    } else if dir.y == 0 {
                        [dir + Vec2 { x: 0, y: 1 }, dir + Vec2 { x: 0, y: -1 }]
                    } else {
                        [Vec2 { x: dir.x, y: 0 }, dir + Vec2 { x: 0, y: dir.y }]
                    };
                    let mut moved = 0.0;
                    for dir2 in dirs {
                        let i2 = (i as i32 + dir2.x + dir2.y * self.size.x as i32) as usize;
                        let move_den =
                            (den * den + vel.mag() * 0.1 - dr[i2] * dr[i2]).max(0.0).min(den) / 2.0;
                        if move_den > 0.0 && !self.barrier[i2] {
                            let sum = move_den + dr[i2];
                            let mov: Vec2<f32> = dir2.into();
                            let new_pos = pos - mov;
                            pm[i2] += (new_pos * move_den + pr[i2] * dr[i2]) / sum - pr[i2];
                            vm[i2] += (vel * move_den + vr[i2] * dr[i2]) / sum - vr[i2];
                            dm[i2] += move_den;

                            moved += move_den;
                        }
                    }
                    den -= moved;
                    pos = Vec2::zero();
                    vel = Vec2::zero();
                }
                pw[i] = pos;
                dw[i] = den;
                vw[i] = vel;
            }
        }
        self.dens.swap();
        self.vel.swap();
        self.pos.swap();
    }
}
